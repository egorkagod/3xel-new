...
export default function GoodPage() {
    ...
    return (
        <div className={classes.globalContainer}>
            <div style={{ width: '100%', height: '100%', backgroundImage: variantId ? `url(${product.variants.find(v => v.id === variantId).image})` : 'none', backgroundSize: 'cover', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', borderRadius: '12px', boxShadow: '2px 2px 10px black' }}></div>
            <div className={classes.infoAboutGood}>
                ...
                <div className={classes.sizes}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                        {uniqueSizesAndCosts.map((variant, index) => <div key={index} onClick={() => { setSelectedSize(variant.size); setCost(variant.price) }} style={{ fontSize: '20px', textDecoration: selectedSize === variant.size ? 'underline' : 'none', cursor: 'pointer' }}>{variant.size} см</div>)}
                    </div>
                </div>
                ...
                <div>
                    {count === 0 ? (<button style={{ display: 'flex', alignItems: 'center', gap: '5px', backgroundColor: 'black', padding: '10px 0', width: '100%' }} className={classes.addToCartBtn} onClick={(e) => { dispatch(addToCart({ id: variantId, title: product.name, background: product.variants.find(v => v.id === variantId).image, selectedColor, selectedSize, cost: product.variants.find(v => v.id === variantId).price })); e.stopPropagation() }}>
                        ...
                        <div style={{ display: count ? 'flex' : 'none', justifyContent: 'center', alignItems: 'center', border: '1px solid black', borderRadius: '100px', padding: '10px 0', gap: '20px', position: 'relative', backgroundColor: 'black', height: 'fit-content', width: '100%' }}>
