                ...
                <div>
                    {count === 0 ? (<button style={{ display: 'flex', alignItems: 'center', gap: '5px', backgroundColor: 'black', padding: '10px 50px' }} className={classes.addToCartBtn} onClick={(e) => { dispatch(addToCart({ id: variantId, title: product.name, background: product.variants.find(v => v.id === variantId).image, selectedColor, selectedSize, cost: product.variants.find(v => v.id === variantId).price })); e.stopPropagation() }}>
                        ...
                        <div style={{ display: count ? 'flex' : 'none', justifyContent: 'space-between', alignItems: 'center', border: '1px solid black', borderRadius: '100px', padding: '10px 50px', gap: '20px', position: 'relative', backgroundColor: 'black', height: 'fit-content' }}>
                            ...
                            <button style={{ all: 'unset', color: 'green', cursor: 'pointer', fontSize: '20px' }} onClick={(e) => { dispatch(addToCart({ id: variantId, title: product.name, background: product.variants.find(v => v.id === variantId).image, selectedColor, selectedSize, cost: product.variants.find(v => v.id === variantId).price })); e.stopPropagation() }}>+</button>
                        </div>
                    )}
                </div>
            </div>
