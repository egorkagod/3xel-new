                            ...
                            {item.name === 'Подарочный сертификат' ? (
                                ...
                                <div className={classes.addedBust}>
                                    <div className={classes.itemDescription} key={index}>
                                        <span style={{ display: 'flex', gap: '20px' }}>{item.name} — {item.size}, цвет: <span style={{ display: 'block', borderRadius: '50%', width: '24px', height: '24px', background: item.color, border: '1px solid black' }}></span> </span>
                                        {item.discount === 0 ? (
